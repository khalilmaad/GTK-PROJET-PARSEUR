...
{
    GtkButton* togglebutton = GTK_BUTTON(obj->Widget_Ptr);

    // Label
    const char* label = get_attribut("label", obj);
    if (label != NULL)
    {
        gtk_button_set_label(togglebutton, label);
    }
    ...
    {
        gtk_button_set_use_underline(togglebutton, to_bool(use_underline));
    }
    ...
    if (type_icone != NULL && icone != NULL)
    {
        if (to_bool(type_icone))
        {
            ...
            gtk_button_set_image(togglebutton, image);
        }
        else
        {
    ...
    if (active != NULL)
    {
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(togglebutton), to_bool(active));
    }
    ...
    {
        gtk_button_set_always_show_image(togglebutton, to_bool(toujours_afficher_icone));
    }
    ...
    if (position_icone != NULL)
    {
        ...
        for (int i = 0; position_mapping[i].nom != NULL; i++)
        {
            ...
            {
                gtk_button_set_image_position(togglebutton, position_mapping[i].position);
                 goto suivant;
            }
        }
